...
def TOPLOG(found_objects, PICK_TARGETS, PLACE_TARGETS, query, max_tasks=12):
    steps_text = []
    combined_scores = {}
    ...
    "It is important that the option be chosen from the set of possible options defined within \\\\'BEGIN OPTIONS AVAILABLES\\\\' through \\\\'END OPTIONS\\\\'."
    "DO NOT USE OTHER OPTIONS THAT ARE NOT PRESENT HERE! You have available also a scene description with the objects observed pickable in the scene, this description is defined within \\\\'BEGIN SCENE DESCRIPTION\\\\' through \\\\'END SCENE DESCRIPTION\\\\'."
    "IMPORTANT: Answer \\'done()\\' when you think the request has been satisfied."
    "Consider that the robot can ONLY perform pick and place operations AND the robot can place MULTIPLE items inside the microwave-oven with the objects detected and only them (do not involve other places): that is, it can only lift objects and reposition them in another place, but it cannot, for example, use tools or perform complex movements. "
    "IMPORTANT: Always prioritize STABLE and SAFE solutions!"
    """

...
def ask_gpt_for_top_5_options(options, query, gpt3_context, found_objects, model="gpt-4o-mini"):
    ...
    The robot can perform only pick and place operation. For example 'robot.pick_and_place(cube, table)' means pick the cube and place it on the table; robot.pick_and_place(tool, user) means give the tool to the user (and then he will use it as he whishes).
    ...
    if len(top_5_options) < 1:
        print(f"Errore: Sono state restituite solo {len(top_5_options)} opzioni. Richiamo GPT per completare...")
        top_5_options.extend(ask_gpt_for_more_options(options, query, 5 - len(top_5_options)))

    
    elif len(top_5_options) > 5:
        print(f"Errore: Sono state restituite più di 5 opzioni ({len(top_5_options)}). Manteniamo solo le prime 5.")
        top_5_options = top_5_options[:5]

    return top_5_options, reasoning


def ask_gpt_for_more_options(options, query, num_missing, model="gpt-4o-mini"):
    system_message = f"""
...
def positive_prompting(top_5_options, query, model="gpt-4o-mini", num_samples=5):
    ...
    Your goal is to choose the options that are most likely to help achieve the user's objective in pick and place operation. For example 'robot.pick_and_place(cube, table)' means pick the cube and place it on the table; robot.pick_and_place(tool, user) means give the tool to the user (and then he will use it as he whishes).
        ...
        if use_bbox:
            ...
            if bounding_box is not None:
                ...
                if max_side > max_gripper_size:
                    ...
                    print(f"Penalità per {pick_object} a causa del lato {max_side} maggiore di {max_gripper_size}: -{penalty:.2f}")
                else:

...
from scripts.experiments.baselines.llm_planner_baseline import (
    build_fewshot_pool,
    run_llm_planner_item,
)
...
def _aggregate_category_rows(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    avg_examples = sum(r["num_fewshot_examples"] for r in rows) / count if count else 0.0
    return {
        "count": count,
        ...
        "done_rate": done_rate,
        "avg_fewshot_examples": avg_examples,
    }


def _load_items_for_categories(categories: List[str]) -> Dict[str, Dict[str, Any]]:
    loaded: Dict[str, Dict[str, Any]] = {}
    for category in categories:
        cat = load_category(category)
        loaded[category] = {
            "category": cat,
            "items": [{**item, "category": category} for item in resolve_items(cat)],
...
def main() -> None:
    ...
    parser.add_argument("--out", default="scripts/experiments/baselines/results/llm_planner_baseline")
    ...
    loaded = _load_items_for_categories(args.categories)
    fewshot_pool = build_fewshot_pool(
        item for category_data in loaded.values() for item in category_data["items"]
    )
    ...
    for category in args.categories:
        category_data = loaded[category]
        items = category_data["items"]
        policy_meta = category_data["policy_meta"]
        ...
        for item in items:
            ...
            for trial in range(1, args.num_trials + 1):
                ...
                row = {
                    ...
                    "num_fewshot_examples": len(result["fewshot_example_ids"]),
                    "fewshot_example_ids": ",".join(result["fewshot_example_ids"]),
                }
                    ...
                    trace_records.append(
                        {
                            ...
                            "fewshot_example_ids": result["fewshot_example_ids"],
                        }
                    )
        ...
        json.dump(
            {
                ...
                "num_examples": args.num_examples,
                "baseline_note": (
                    "LLM-Planner-style adapted baseline using ConceptBot instructions, "
                    "visible objects, valid destinations, and retrieved few-shot examples. "
                    "This is not the official ALFRED/THOR implementation."
                ),
            },
            f,
            indent=2,
    ...
    print("Average few-shot examples:", overall["avg_fewshot_examples"])
